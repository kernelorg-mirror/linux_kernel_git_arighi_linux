...
static struct sched_group *sched_balance_find_dst_group(struct sched_domain *sd,
 ...
 */
static int sched_balance_find_dst_group_cpu(struct sched_group *group, struct task_struct *p,
					    int this_cpu, const struct cpumask *cpus_allowed)
{
...
/*
 * __select_task_rq_fair: Select a target runqueue for the waking task from
...
__select_task_rq_fair(struct task_struct *p, int prev_cpu, int wake_flags,
			      const struct cpumask *cpus_allowed)
{
	const struct cpumask *allowed = cpus_allowed ?: p->cpus_ptr;
	...
	if (wake_flags & WF_TTWU) {
		...
		if ((wake_flags & WF_CURRENT_CPU) && cpumask_test_cpu(cpu, allowed))
			return cpu;
	...
	/* Slow path */
	if (unlikely(sd))
		return sched_balance_find_dst_cpu(sd, p, cpu, prev_cpu, sd_flag, cpus_allowed);
...
int select_task_rq_fair_mask(struct task_struct *p, int prev_cpu, int wake_flags,
			     const struct cpumask *cpus_allowed)
{
	return __select_task_rq_fair(p, prev_cpu, wake_flags, cpus_allowed);
}
...
static inline void update_sg_wakeup_stats(struct sched_domain *sd, struct sched_group *group,
					  struct sg_lb_stats *sgs, struct task_struct *p,
					  const struct cpumask *cpus_allowed)
{
	int i, nr_running;
							...
							const struct cpumask *cpus_allowed)
{
	...
	do {
		...
		if (!cpumask_intersects(sched_group_span(group), allowed))
			continue;
	...
	for_each_domain(cpu, sd) {
		...
		if (time_after_eq(jiffies, sd->last_balance + interval)) {
			if (!fair_ext_called && fair_ext_balance_enabled()) {
				...
				fair_ext_handled = fair_ext_balance(cpu, idle);
			}
			if (!fair_ext_handled &&
	...
	__must_hold(__rq_lockp(this_rq))
{
	...
	if (fair_ext_balance_enabled())
		fair_ext_handled = fair_ext_balance(this_cpu, CPU_NEWLY_IDLE);
