...
static bool dispatch_rescue(struct task_struct *p, task_ctx_t *taskc,
			    u64 enq_flags)
{
	s32 cid = cmask_next_set_wrap(&taskc->cpus_allowed, 0);

	if (cid < 0 || cid >= scx_bpf_nr_cids())
		return false;
...
void BPF_STRUCT_OPS(qmap_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (!cmask_intersects(&taskc->cpus_allowed, &qa.self_cids.mask) &&
	    dispatch_rescue(p, taskc, enq_flags))
		return;
	...
	if (enq_flags & SCX_ENQ_BLOCKED) {
		cid = cmask_next_and_set_wrap(&taskc->cpus_allowed,
					      ...
					      scx_bpf_task_cid(p));
		if (cid >= scx_bpf_nr_cids()) {
			if (!dispatch_rescue(p, taskc, enq_flags))
				scx_bpf_error("blocked donor has no allowed cid");
			return;
		}
		scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL_ON | cid, slice_ns,
