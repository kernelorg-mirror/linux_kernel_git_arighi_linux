...
#include "scx_fair_vlag.bpf.skel.h"

static const char help_fmt[] =
"A fair_ext policy which resets virtual lag when placing tasks.\n"
"\n"
"Usage: %s [-v]\n"
"\n"
...
static int read_stats(struct scx_fair_vlag *skel,
		      struct scx_fair_vlag_stats *totals)
{
	int nr_cpus = libbpf_num_possible_cpus();
	struct scx_fair_vlag_stats *values;
	__u32 idx = 0;
	int cpu, ret;
...
{
	struct scx_fair_vlag *skel = NULL;
	...
	skel = scx_fair_vlag__open_and_load();
	if (!skel) {
		fprintf(stderr, "failed to open and load BPF programs\n");
		return 1;
	}

	link = bpf_map__attach_struct_ops(skel->maps.scx_fair_vlag_ops);
	err = libbpf_get_error(link);
	if (err) {
		link = NULL;
		fprintf(stderr, "failed to attach scx_fair_vlag: %s\n",
			strerror(-err));
		goto out;
	}

	printf("scx_fair_vlag attached\n");
	while (!exit_req) {
