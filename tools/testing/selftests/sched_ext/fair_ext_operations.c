...
#define SCX_NE(_x, _y) \
	SCX_FAIL_IF((_x) == (_y), "Expected %s != %s", #_x, #_y)
...
static int child_fn(int wake_fd, int result_fd)
{
	...
	for (;;) {
		...
		/* Consume enough runtime to force at least one request renewal. */
...
static enum scx_test_status test_operations(void *arg)
{
	...
	__u64 calls, count, empty_count, renew_count;
	...
	SCX_EQ(0, err);
	SCX_EQ(SCHED_OTHER, sched_getscheduler(ctx->child));

	ctx->skel->data->target_pid = ctx->child;
	ctx->skel->data->target_cpu = cpu1;
	count = ctx->skel->bss->nr_overrides;
	observed = wake_child(ctx->child, ctx->wake_pipe[1],
			      ctx->result_pipe[0]);
	SCX_GT(ctx->skel->bss->nr_overrides, count);
	SCX_EQ(cpu1, observed);

	/* An out-of-range virtual lag must retain fair's proposal. */
	ctx->skel->bss->invalid_vlag = true;
	count = ctx->skel->bss->nr_vlag_invalid;
	observed = wake_child(ctx->child, ctx->wake_pipe[1],
			      ctx->result_pipe[0]);
	SCX_GT(ctx->skel->bss->nr_vlag_invalid, count);
	SCX_EQ(cpu1, observed);
	ctx->skel->bss->invalid_vlag = false;

	/* Select zero virtual lag for placement and renewal. */
	ctx->skel->bss->use_vlag = true;
	count = ctx->skel->bss->nr_vlag_select;
	renew_count = ctx->skel->bss->nr_vlag_renew;
	observed = wake_child(ctx->child, ctx->wake_pipe[1],
			      ctx->result_pipe[0]);
	SCX_GT(ctx->skel->bss->nr_vlag_select, count);
	SCX_GT(ctx->skel->bss->nr_vlag_place, 0);
	SCX_GT(ctx->skel->bss->nr_vlag_renew, renew_count);
	SCX_EQ(cpu1, observed);
	ctx->skel->bss->use_vlag = false;
	...
	observed = wake_child(ctx->child, ctx->wake_pipe[1],
			      ctx->result_pipe[0]);
	SCX_GT(ctx->skel->bss->nr_fair_mask, count);
	SCX_GT(ctx->skel->bss->nr_cpu_idle, 0);
	SCX_ASSERT(observed == cpu0 || observed == cpu1);
	...
	if (cpu2 >= 0) {
		...
		observed = wake_child(ctx->child, ctx->wake_pipe[1],
				      ctx->result_pipe[0]);
		SCX_GT(ctx->skel->bss->nr_fair_mask, count);
		SCX_GT(ctx->skel->bss->nr_fair_mask_empty, empty_count);
		SCX_ASSERT(observed == cpu0 || observed == cpu1);
		ctx->skel->bss->use_fair_mask = false;
	}
	...
	observed = wake_child(ctx->child, ctx->wake_pipe[1],
			      ctx->result_pipe[0]);
	SCX_GT(ctx->skel->bss->nr_fallbacks, count);
	SCX_ASSERT(observed == cpu0 || observed == cpu1);
	SCX_GT(ctx->skel->bss->nr_balance, 0);
	SCX_GT(ctx->skel->bss->nr_idle_queries, 0);
...
struct scx_test fair_ext_operations = {
	.name = "fair_ext.operations",
	.description = "Verify fair sched_ext callbacks and native fallback",
	.setup = setup,
	.run = test_operations,
	.cleanup = cleanup,
};

REGISTER_SCX_TEST(&fair_ext_operations)
