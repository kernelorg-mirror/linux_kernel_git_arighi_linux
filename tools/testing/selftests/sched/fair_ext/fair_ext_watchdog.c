...
static enum fair_ext_test_status test_watchdog(void *arg)
{
	struct fair_ext_watchdog_ctx *ctx = arg;
	cpu_set_t allowed;
	char state[16];
	...
	for (int i = 0; i < 50; i++) {
		...
		if (!probe)
			_exit(0);
		FAIR_EXT_EQ(probe, waitpid(probe, &status, 0));
		FAIR_EXT_ASSERT(WIFEXITED(status));
	...
	FAIR_EXT_ASSERT(disabled);
	FAIR_EXT_EQ(0, read_fair_ext_state(state, sizeof(state)));
	FAIR_EXT_FAIL_IF(strcmp("faulted", state), "Unexpected state: %s", state);
	calls = ctx->skel->bss->nr_calls;
	usleep(50000);
	...
	ctx->link = NULL;
	FAIR_EXT_EQ(0, read_fair_ext_state(state, sizeof(state)));
	FAIR_EXT_FAIL_IF(strcmp("disabled", state), "Unexpected state: %s", state);

	return FAIR_EXT_TEST_PASS;
}

struct fair_ext_test fair_ext_watchdog = {
	.name = "fair_ext.watchdog",
	.description = "Verify sched_fair_ops watchdog enforcement",
	.setup = setup,
	.run = test_watchdog,
	.cleanup = cleanup,
};

REGISTER_FAIR_EXT_TEST(&fair_ext_watchdog)
